...
class Specs(Dataset):
    def __init__(self, data_dir, subset, dummy, shuffle_spec, num_frames,
        ...
        if format == "default":
            ...
            # Fallback for flat DNS-style layout:
            #   base_dir/train_clean, base_dir/train_noisy,
            #   base_dir/dev_clean,   base_dir/dev_noisy,
            #   base_dir/test_clean,  base_dir/test_noisy
            if len(self.clean_files_all) == 0 and len(self.noisy_files_all) == 0:
                flat_subset = "dev" if subset == "valid" else subset
                self.clean_files_all += sorted(glob(join(data_dir, f"{flat_subset}_clean", "*.wav")))
                self.clean_files_all += sorted(glob(join(data_dir, f"{flat_subset}_clean", "**", "*.wav")))
                self.noisy_files_all += sorted(glob(join(data_dir, f"{flat_subset}_noisy", "*.wav")))
                self.noisy_files_all += sorted(glob(join(data_dir, f"{flat_subset}_noisy", "**", "*.wav")))
